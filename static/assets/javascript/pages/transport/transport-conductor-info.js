    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                const search_url = "/apitransport-conductor-api/";
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        { data: 'conductor_name' },
                        { data: 'contact_no' },
                        { data: 'con_present_address' },
                        { data: 'con_permanent_address' },
                        { data: 'license_no' },
                        {
                            "data": null,
...
$(function () {
    ...
    function show_edit_form(id) {
        $.ajax({
            url: '/transport-conductor-edit/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
...
function get_district(id){
    $.ajax({
        url: '/apiauth-district-api/?division_id='+ id,
        type: 'get',
        dataType: 'json',
        success: function (data) {
            var district = $('#id_pre_district_id')
            $('#id_pre_district_id option').remove();
            district.append('<option value="">----------</option>')
            data.forEach(e => {
                district.append('<option value="'+e.district_id+'">'+e.district_name+'</option>')
            });
        }
    })
}
$('#id_pre_district_id').change(function () {
    get_upazila(this.value);
});
function get_upazila(id){
    $.ajax({
        url: '/apiauth-upazila-api/?district_id='+ id,
        type: 'get',
        dataType: 'json',
        success: function (data) {
            var district = $('#id_pre_upozila_id')
            $('#id_pre_upozila_id option').remove();
            district.append('<option value="">----------</option>')
            data.forEach(e => {
                district.append('<option value="'+e.upozila_id+'">'+e.upozila_name+'</option>')
            });
        }
    })
}
$('#id_per_division_id').change(function () {
    // district_id
    get_district_per(this.value);
});
function get_district_per(id){
    $.ajax({
        url: '/apiauth-district-api/?division_id='+ id,
        type: 'get',
        dataType: 'json',
        success: function (data) {
            var district = $('#id_per_district_id')
...
$('#id_per_district_id').change(function () {
    get_upazila_per(this.value);
});
function get_upazila_per(id){
    $.ajax({
        url: '/apiauth-upazila-api/?district_id='+ id,
        type: 'get',
        dataType: 'json',
        success: function (data) {
            var district = $('#id_per_upozila_id')
