            ...
            value: function table() {
                ...
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'leave_reason' },
                        { data: 'application_to.employee_name' },
                        { data: 'approve_by' },
                        { data: 'approval_comments' },
                        {
                            "data": null,
                            "defaultContent": '<button type="button" class="btn btn-info btn-sm edit">Edit</button>'
                            
                        },
                        {
                            "data": null,
                            "defaultContent": '<button type="button" class="btn btn-info btn-sm aprove">aprove</button>'
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        if (class_name == 'btn btn-info btn-sm edit') {
            show_edit_form(id);
        }
        if (class_name == 'btn btn-info btn-sm aprove'){
            show_approve_form(id);
        }
    ...
    function show_approve_form(id) {
        $.ajax({
            url: '/hrm-employee-leave-application-approve/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $('#edit_model').modal('show');
...
$(document).ready(function() {
    $('#id_employee_id').select2({placeholder: " Select a Employee "});
    $('#id_application_to').select2({placeholder: " Select a Applicant "});
});
