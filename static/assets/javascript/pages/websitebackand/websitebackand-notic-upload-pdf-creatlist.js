    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                console.log('test')
                var notic_title = document.getElementById('id_notic_title').value;
                var pdf_file = document.getElementById('id_pdf_file').value;
                var status = document.getElementById('id_status').value;
                var education_lavel = document.getElementById('id_education_lavel').value;
                var types = document.getElementById('id_types').value;
                

                const search_url = "/notic_upload_pdf-api/?notic_title=" + notic_title + "&pdf_file=" + pdf_file+ "&status" +status +"&education_lavel" +education_lavel+"&types" +types;
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        if (class_name == 'btn btn-warning btn-sm') {
            if (confirm('Are you sure you want to remove this PDF file?') == true) {
                delete_customer(id)
            }
        }
    })
    ...
    function show_edit_form(id) {
        $.ajax({
            url: '/websitebackend-upload-pdf-edit/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
            ...
            if (data.form_is_valid) {
                ...
                if(data.success_message){
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
                        title: data.success_message+" "+data.department_id,
