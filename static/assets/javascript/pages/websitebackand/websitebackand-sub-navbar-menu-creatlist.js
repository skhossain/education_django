    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                console.log('test')
                var navbar_lavel = document.getElementById('id_navbar_lavel').value;
                var menu_lavel = document.getElementById('id_menu_lavel').value;
                var menu_url = document.getElementById('id_menu_url').value;
                var serial = document.getElementById('id_serial').value;
                var status = document.getElementById('id_status').value;

                const search_url = "/apiwebsite-navbarsubmenu-api/?navbar_lavel="+ navbar_lavel + "&menu_lavel" + menu_lavel + "&menu_url=" + menu_url+ "&serial" +serial +"&status" +status;
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        if (class_name == 'btn btn-warning btn-sm') {
            if (confirm('Are you sure you want to remove this Sub-Navbar?') == true) {
                delete_customer(id)
            }
        }
    })
    ...
    function show_edit_form(id) {
        $.ajax({
            url: '/websitebackend-navbar-sub-menu-edit/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
    ...
    function delete_customer(id) {
        $.ajax({
            url: '/websitebackend-navbar-sub-menu-delete/' + id,
            type: 'POST',
            dataType: 'json',
            success: function (data) {
