    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                const bed_name = document.getElementById('id_bed_name').value;
                const search_url = "/apihostel-bedmanagement-api/?bed_name="+bed_name;
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        { data: 'bed_type_id.bed_type_name' },
                        { data: 'bed_name' },
                        { data: 'bed_price' },
                        {
                            "data": null,
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        try {
            ...
            id = table_row['beds_management_id']
        }
        catch (e) {
    ...
    function show_edit_form(id) {
        $.ajax({
            url: '/hostel-bedmanagement-edit/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
...
// Select 2
$('#id_bed_type_id').select2({placeholder: "Select a bed type"})
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.form_is_valid) {
                ...
                $('#id_bed_type_id').val('').trigger('change')
            } else {
                $('#page_loading').modal('hide');
