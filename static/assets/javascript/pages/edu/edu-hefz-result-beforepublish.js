...
function all_student_result_sub() {
    ...
    $.ajax({
        url: 'edu-hefz-result-before-publish-data',
        type: 'POST',
        data:postdata,
        datatype:'jason',
        success: function (data) {
...
function list_view() {
    ...
    let url = data_url + '?branch_code='+branch_code+'&academic_year='+academic_year+'&term_id=' + term_id +'&limit=' + limit +'&page=' + page +'&class_name=' + class_id
    ...
    $.ajax({
        ...
        success: function (data) {            
            data.students_results.forEach((student_result, index) => {
                ...
                pdf.text(":", 40, 53);
                pdf.setFont(undefined, 'normal')
                pdf.text(student_result.student_roll__student_father_name?student_result.student_roll__student_father_name:"", 45, 53);

                pdf.setFont(undefined, 'bold')
                pdf.text("Mother's Name", 10, 57);
                pdf.text(":", 40, 57);
                pdf.setFont(undefined, 'normal')
                pdf.text(student_result.student_roll__student_mother_name?student_result.student_roll__student_mother_name:"", 45, 57);
                ...
                pdf.text(":", 40, 65);
                pdf.setFont(undefined, 'normal')
                pdf.text(data.class_name, 45, 65);
                ...
                data.subject_results.forEach(subResult => {
                    if (subResult.student_roll == student_result.student_roll) {
                        ...
                        pdf.text(subResult.gpa, 120, y+92);
                        pdf.line(132, y+85, 132, y+95)
                        pdf.text(subResult.lg, 135, y+92);
                        pdf.line(148, y+85, 148, y+95)
                        pdf.text(subResult.subject_id__maximum_marks?subResult.subject_id__maximum_marks.toString():"", 150, y+92);
                ...
                pdf.text((student_result.gpa).toString(), 120, y+92);
                pdf.line(132, y+85, 132, y+95)
            ...
            pdf.save('Result- Class '+data.class_name+data.page_info);
            loder_Spinner(false)
        }
    })
}
