    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                const search_url = "/apiedu-examattendance-api/";
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            "data": null,
                            "defaultContent": '<button type="button" class="btn btn-info btn-sm">Edit</button> <button type="button" class="atten btn btn-info btn-sm">View sheet</button> <button type="button" class="pdf btn btn-info btn-sm">PDF sheet</button>'
                        }
                    ]
                });
            }
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        if (class_name == 'atten btn btn-info btn-sm') {
            attendance_view_form(id);
        }
        if (class_name == 'pdf btn btn-info btn-sm') {
            attendance_pdf_view(id);
        }
    })

    function show_edit_form(id) {
        $.ajax({
            url: '/edu-examattendance-editform/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
    ...
    function attendance_view_form(id) {
        $.ajax({
            url: '/edu-examattendance-viewform/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $('#edit_model').modal('show');
    ...
    function attendance_pdf_view(id) {
        $.ajax({
            url: '/edu-examattendance-pdfview/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $('#edit_model').modal('show');
