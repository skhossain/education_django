    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                const search_url = "/apiedu-librarybookrequest-api/";
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'card_number.name_on_card' },
                        { data: 'request_date' },
                        { data: 'actual_return_date' },
                        { data: 'day_to_return_book' },
                        { data: 'book_issue_status' },
                        {
                            "data": null,
...
$(function () {
    ...
    function show_edit_form(id) {
        $.ajax({
            url: '/edu-librarybookrequest-edit/' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
...
// Select 2
$('#id_book_id').select2({placeholder: "Select a book"})
$('#id_card_number').select2({placeholder: "Select card"})
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.form_is_valid) {
                ...
                $('#id_book_id').val('').trigger('change')
                $('#id_card_number').val('').trigger('change')
            } else {
                $('#page_loading').modal('hide');
